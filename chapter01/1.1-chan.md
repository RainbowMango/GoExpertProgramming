...
3. 将待发送数据写入G，将当前G加入sendq，进入睡眠，等待被读goroutine唤醒；
...
# 4. 常见用法
# 4.1 单向channel
顾名思义,单向channel指只能用于发送或接收数据，这种channel是没有实际用途的，实际上也没有单向channel。
...
通过这个示例想说的是：select的case语句读channel不会阻塞，尽管channel中没有数据。这是由于case语句编译后调用读channel时会明确传入不阻塞的参数，此时读不到数时不会将当前goroutine加入到等待队列，而是直接返回。
